...
public class OrderServiceImpl extends ServiceImpl<OrderInfoMapper, OrderInfo> implements OrderService {
    ...
    public Long saveOrderInfo(OrderInfo orderInfo) {
        ...
        orderInfo.sumTotalAmount();
        //userId在控制器能获取到，暂时不用写
        //订单状态
        orderInfo.setOrderStatus(OrderStatus.UNPAID.name());
        //第三方交易编号-给支付宝使用的，能够保证幂等性
...
// 发送消息队列：发送延迟消息,如果在规定的时间内未付款，那么就取消订单。根据订单Id取消。
    ...
    public OrderInfo getOrderInfo(Long orderId) {
        ...
        List<OrderDetail> orderDetailsList = orderDetailMapper.selectList(queryWrapper);
        orderInfo.setOrderDetailList(orderDetailsList);
        return orderInfo;
    }

    //通过orderId发消息给库存，减少库存
    @Override
    public void sendOrderStatus(Long orderId) {
        //更新订单状态
        updateOrderStatus(orderId, ProcessStatus.NOTIFIED_WARE);
        //发送一个json字符串，通过接口文档得知发送的数据都是从OrderInfo 中获取的部分字段组成的json。
    ...
    public List<OrderInfo> orderSplit(long orderId, String wareSkuMap) {
        /*
         * 1，先获取原始订单，谁要被拆单
         * 2，将wareSkuMap 参数转化为我们程序可以操作ode订单
         * 3，创建一个新的子订单
         * 4，给子订单进行赋值
         * 5，保存子订单
         * 6，将子订单添加到集合
